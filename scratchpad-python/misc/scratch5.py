...
def categorize(inputData):
    ...
    for seq, row in enumerate(inputData):
        ...
        if seq == 0:
            pattern = r'\s*(?P<fromyear>\d{4})\s*(-\s*(?P<frommonth>\d+)\s*)*(-\s*(?P<fromdate>\d+)\s*)*,\s*(?P<toyear>\d{4})\s*(-\s*(?P<tomonth>\d+))*(-\s*(?P<todate>\d+)\s*)*'
            ...
            if regex is not None:
                ...
                try:
                    if fromDates is not None and fromMonth is not None:
                        fromDate = datetime.date(int(fromYear), int(fromMonth), int(fromDates))  # start date, month are specified
            ...
            # Parse engagement data
            pattern = r'\s*(?P<year>\d{4})\s*-\s*(?P<month>\d+)\s*(-\s*(?P<date>\d+)\s*)*,\s*(?P<engagement>.[^,]+)\s*,\s*(?P<number>-*\d+)'
            regex = re.match(pattern, row)
