            ...
            if regex is not None:  # if not according to pattern, pass
                ...
                if (toDate > dataDate > fromDate) and number > 0:
                    ...
                    if keyDate not in outputData:
                        outputData[keyDate] = [engagement]
                    else:
                        for eng in outputData[keyDate]:
                            if eng['name'] == engagement['name']:
                                eng['number'] += engagement['number']
                                break
                        else:
                            outputData[keyDate].append(engagement)

    for key, value in outputData.items():
        outputData[key] = sorted(value, key=lambda x: x['name'])
...
def printByMonth(outputData):
    ...
    for key in sorted(outputData.keys(), reverse=True):
        ...
        for i in outputData[key]:
            if i['number'] > 0:
                string += ', %s, %d' % (i['name'], i['number'])
        print(string)
